...
// Where a line may break: whitespace, and the separators project names use.
const BREAK = /[\s\-_/.]/;
...
function wrap(text, max, maxLines) {
  ...
  while (rest && out.length < maxLines) {
    ...
    // If the text continues at a break character, `take` already ends on a
    // boundary — backing up would waste a whole word.
    const endsCleanly = rest.length === take.length || BREAK.test(rest[take.length]);
    if (!endsCleanly) {
      // Project names have no spaces (`billing-api`, `search_service`), so a
      // hyphen, underscore, slash or dot is a break too — without them a name
      // splits mid-word and reads like a typo.
      let cut = -1;
      for (let i = take.length - 1; i > max * 0.4; i--) {
        if (BREAK.test(take[i])) { cut = i; break; }
      }
      // Nothing past the 40% mark: an early break still beats splitting a word
      // in half, which reads like a typo ("agent_framewor" / "k").
      if (cut < 0) {
        for (let i = take.length - 1; i > 0; i--) {
          if (BREAK.test(take[i])) { cut = i; break; }
        }
      }
      // Keep the separator on the first line unless it is a space.
      if (cut > 0) take = take.slice(0, take[cut] === ' ' ? cut : cut + 1);
    }
