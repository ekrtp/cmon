...
// CLAUDE_MONITOR_CONFIG points this module somewhere else. The test suite uses
// it so a test run never touches the config of a monitor you have open — an
// earlier version wrote a theme into the real file and every running board
// changed colour mid-test.
const FILE = process.env.CLAUDE_MONITOR_CONFIG ||
  path.join(os.homedir(), '.claude', 'monitor', 'config.json');
...
const DEFAULTS = {
  ...
  zebra: true,                   // shade alternate rows so a wrapped row reads as one
  zebraStrength: 0.07,           // 0 = invisible, 0.2 = obvious
...
function sanitise(raw) {
  ...
  const strength = Number(c.zebraStrength);
  c.zebraStrength = strength >= 0 && strength <= 0.4 ? strength : DEFAULTS.zebraStrength;
  c.zebra = c.zebra !== false;
